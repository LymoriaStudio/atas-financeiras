...
export function AdminCategories() {
  ...
  const save = async () => {
    ...
    if (modal === "add") {
      ...
      else { setErrorMsg("Erro ao criar categoria. Tente novamente."); setSubmitting(false); return; }
  ...
  <div className="overflow-x-auto">
    <table style={{ width: "100%", minWidth: 640, borderCollapse: "collapse" }}>
      <colgroup>
        ...
        <col style={{ width: "10%" }} />   {/* Documentos */}
        <col style={{ width: "25%" }} />   {/* Participação */}
            ...
            >
              ...
              <td style={{ padding: "16px 24px" }}>
                <div className="flex items-center gap-2.5">
                  ...
                  >
                    {iconMap[cat.icon]
                      ? <span style={{ transform: "scale(0.7)", display: "flex" }}>{iconMap[cat.icon]}</span>
                      : null}
                  </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6">
            ...
            <div className="flex items-center gap-3 mb-5 p-4 rounded-xl border border-gray-100 bg-gray-50">
              <div
                ...
                style={{ backgroundColor: `${viewingCat.color}15`, color: viewingCat.color }}
              >
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between py-2 border-b border-gray-50">
                ...
                <span className="font-semibold text-gray-800">{categoriaCounts[viewingCat.id] ?? 0}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-50">
                <span className="text-gray-400">Participação</span>
                <span className="font-semibold text-gray-800">
                  {totalAtas > 0 ? Math.round(((categoriaCounts[viewingCat.id] ?? 0) / totalAtas) * 100) : 0}%
                </span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-50">
                <span className="text-gray-400">Slug</span>
                <span className="font-mono text-xs text-gray-600 bg-gray-100 px-2 py-0.5 rounded">{viewingCat.slug}</span>
              </div>
              <div className="flex justify-between py-2">
                <span className="text-gray-400">Cor</span>
                <span className="flex items-center gap-2">
                  <span className="w-4 h-4 rounded-full inline-block border border-gray-200" style={{ backgroundColor: viewingCat.color }} />
                  <span className="font-mono text-xs text-gray-600">{viewingCat.color}</span>
                </span>
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              ...
              <button
                onClick={() => { close(); openEdit(viewingCat); }}
