...
export function AdminLogin({ onLogin, onBack }: AdminLoginProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      if (email === "admin@sbs.com.br" && password === "admin123") {
        onLogin();
      } else {
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: "#F9FAFB" }}>
      ...
      >
        ...
        <div>
          <h2 className="text-white mb-3" style={{ fontSize: "2rem", fontWeight: 700, lineHeight: 1.2 }}>
            Painel de<br />Administração
          </h2>
          <p className="text-gray-400 text-sm leading-relaxed max-w-sm">
            Gerencie atas, documentos financeiros e estatutos do Portal de Transparência da SBS Participações.
          </p>
        </div>
        <div className="flex gap-6">
          {[["18", "Atas publicadas"], ["3", "Categorias"], ["2026", "Ano atual"]].map(([val, label]) => (
            <div key={label}>
              <p className="text-white font-bold" style={{ fontSize: "1.5rem" }}>{val}</p>
      ...
      <div className="flex-1 flex flex-col justify-center items-center px-8">
        <div className="w-full max-w-sm">
          ...
          <h1 style={{ color: "#111827", fontSize: "1.5rem", fontWeight: 700 }} className="mb-1">
            Entrar no painel
          </h1>
          <p className="text-gray-400 text-sm mb-8">Use suas credenciais de administrador</p>
          ...
          <p className="text-center text-gray-400 text-xs mt-8">
            Credenciais de demo: <span className="text-gray-600 font-medium">admin@sbs.com.br</span> / <span className="text-gray-600 font-medium">admin123</span>
          </p>
        </div>
      </div>
    </div>
